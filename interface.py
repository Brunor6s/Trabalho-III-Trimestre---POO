...
class Interface:
    ...
    def tela_cadastrar_cliente(self):
        ...
        tk.Label(self.janela, text="Senha:").pack()
        senha = tk.Entry(self.janela, show="*"); senha.pack()
        tk.Label(self.janela, text="CPF (11 dígitos):").pack()
        cpf = tk.Entry(self.janela); cpf.pack()
        ...
        tk.Button(self.janela, text="Voltar", 
                  command=(self.menu_dono if self.usuario_logado["tipo"]=="dono" else self.menu_func)).pack()

    # Tela para cadastrar funcionário
    def tela_cadastrar_funcionario(self):
        ...
        def salvar():
            try:
                novo = self.funcService.criar(nome.get(), email.get(), senha.get())
                messagebox.showinfo("OK", "Funcionário cadastrado com sucesso!")
            except Exception as e:
                messagebox.showerror("Erro", str(e))
                return
            self.menu_dono()
    ...
    def tela_cadastrar_quarto(self):
        ...
        numero = tk.Entry(self.janela); numero.pack()
        tk.Label(self.janela, text="Preço:").pack()
        preco = tk.Entry(self.janela); preco.pack()

        def salvar():
            try:
                novo = self.quartoService.criar(numero.get(), preco.get())
                messagebox.showinfo("OK", "Quarto cadastrado com sucesso!")
            except Exception as e:
                messagebox.showerror("Erro", str(e))
                return
            self.menu_dono()
    ...
    def listar_clientes(self):
        ...
        for c in self.clienteService.clientes:
            tk.Label(self.janela, text=f"{c.nome} - {c.email} - CPF: {c.documento}").pack()
        tk.Button(self.janela, text="Voltar",
    ...
    def listar_quartos(self):
        ...
        for q in self.quartoService.quartos:
            tk.Label(self.janela, text=f"Quarto {q.numero} - Tipo: {q.tipo} - R$ {q.precoDiaria:.2f}").pack()
        tk.Button(self.janela, text="Voltar",
                  command=(self.menu_dono if self.usuario_logado["tipo"]=="dono" else self.menu_func)).pack(pady=10)
